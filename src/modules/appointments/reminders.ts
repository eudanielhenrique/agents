...
import { runWhazingAgentNudge } from "@/modules/whazing/nudge";
...
export async function appointmentReminderHandler(
  ...
  // Whazing threadIds have no single decodable shape (ticket/contact-channel/phone — see
  // thread-keys.ts) but all share the "tenant:{tenantId}:whazing:{instanceId}:..." prefix, so the
  // tenant fence just reads the second segment instead of using the Chatwoot 3-segment parser.
  const isWhazing = threadId.includes(":whazing:");
  if (isWhazing) {
    let threadTenantId: bigint | null = null;
    try {
      threadTenantId = BigInt(threadId.split(":")[1] ?? "");
    } catch {
      threadTenantId = null;
    }
    if (threadTenantId === null || threadTenantId !== job.tenantId) {
      return { outcome: "done" };
    }
  } else {
  ...
  if (isWhazing) {
    await runWhazingAgentNudge({ tenantId, threadId, nudge, base, deps });
  } else {
